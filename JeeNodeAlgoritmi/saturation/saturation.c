    ...
    switch (g_message.header) {
      ...
      case Wakeup:
        ...
        switch (g_status) {
            case AVAILABLE:
              DS("Wakeup");
              pushOutboxQueue(Wakeup, 0, &g_message, sizeof(int));
              //D(count_neighbours());
              if (count_neighbours() > 1){
                set_status(ACTIVE);  
              } else {
                set_status(PROCESSING); //In case the node is a leaf (has only 1 neighbour)
                pushOutboxQueue(M, 0, &g_message, sizeof(int));
              }
              break;
            default:
              //DS("Do something");
      ...
      case M:
        ...
        switch (g_status) {
          ...
          case ACTIVE:
            ...
            {
              remove_neighbour(g_message.source);
            }
            if (count_neighbours() == 1){
              //D(find_neighour());
              pushOutboxQueue(M, find_neighour(), &g_message, sizeof(int));
